$(function() {
  ...
  /*-----------------------------------*/
    ...
    $('header .container .menu').append('<div class="menuClose"><a href="">關閉Close</a></div>');
    ...
    _loginInfo.clone().prependTo(_mArea);
    ...
    function mobileMenu() {
      ...
      if (ww < wwMedium) {
        /*-----------------------------------*/
            ...
            $('.m_search').hide();
            _body.removeClass('noscroll');
            // _loginInfo.prependTo('.header .container');
            ...
            search_mode = false;
            _searchbt.off().on('click', function(e) {
              if (!search_mode) {
                $('.header .search').stop(true, false).fadeIn('400', 'easeOutQuint');
                search_mode = true;
              } else {
                $('.header .search').hide();
                search_mode = false;
            ...
            function close_pc_menu() {
              _menu.fadeOut();
              _body.removeClass('noscroll');
            }
            _menubt.off().click(function(e) {
              open_pc_menu();
              _menu.find('a:first').focus();
              e.preventDefault();
            });
            $(".header .menu .menuClose").off().click(function(e) {
              close_pc_menu();
              e.preventDefault();
            });
            $(".header .menu").find('a:last').keyup(function(e) {
              close_pc_menu()
              $('.header .search').css('display', 'block').find('input[type="text"]').focus();
              _menubt.blur();
            });
            _body.off('touchmove');
          };
